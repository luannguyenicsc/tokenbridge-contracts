...
async function sendRawTx({ data, nonce, to, privateKey, url, gasPrice, value, chainId }) {
  console.log('sendRawTx chainId', chainId)
  try {
    ...
    const estimatedGas = BigNumber(await sendNodeRequest(url, 'eth_estimateGas', txToEstimateGas, chainId))

    const blockData = await sendNodeRequest(url, 'eth_getBlockByNumber', ['latest', false], chainId)
    ...
    const txHash = await sendNodeRequest(url, 'eth_sendRawTransaction', `0x${serializedTx.toString('hex')}`, chainId)
    console.log('pending txHash', txHash)
    return await getReceipt(txHash, url, chainId)
  } catch (e) {
    console.error(e)
  }
}

async function sendNodeRequest(url, method, signedData, chainId) {
  if (!Array.isArray(signedData)) {
    signedData = [signedData]
  }
  console.log('sendNodeRequest method', method, chainId)
...
async function getReceipt(txHash, url, chainId) {
  await timeout(GET_RECEIPT_INTERVAL_IN_MILLISECONDS)
  let receipt = await sendNodeRequest(url, 'eth_getTransactionReceipt', txHash, chainId)
  if (receipt === null || receipt.blockNumber === null) {
    receipt = await getReceipt(txHash, url, chainId)
  }
  return receipt
}
...
async function upgradeProxy({ proxy, implementationAddress, version, nonce, url, chainId}) {
  ...
  console.log('chainIdchainIdchainIdchainIdchainIdchainIdchainIdchainIdchainIdchainIdchainId', chainId);
...
async function transferOwnership({ contract, newOwner, nonce, url, chainId}) {
...
async function setBridgeContract({ contract, bridgeAddress, nonce, url, chainId }) {
